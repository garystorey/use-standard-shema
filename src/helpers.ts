...
function applyFlattening<T>(
  ...
  if (isPlainObject(currentValue)) {
    Object.assign(
      resultMap,
      recurseFn(currentValue as FormDefinition, currentPath)
    );
  }
}
...
export function flattenFormDefinition(
  ...
  for (const [propertyKey, propertyValue] of Object.entries(
    formDefinition as Record<string, unknown>
  )) {
    ...
    applyFlattening<FieldDefinition>(
      ...
      (subSchema: FormDefinition, path: string) =>
        flattenFormDefinition(subSchema, path)
    );
  }
...
export function flattenDefaults(
  ...
  for (const [propertyKey, propertyValue] of Object.entries(
    ...
    applyFlattening<string>(
      ...
      (subSchema: FormDefinition, path: string) =>
        flattenDefaults(subSchema, path)
    );
  }
