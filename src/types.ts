...
/** Public aliases */
export type DotPaths<T, Prev extends string = "", D extends Depth = 10> =
  DotFold<T, Prev, "paths", D>

type DotPathsToValues<T, Prev extends string = "", D extends Depth = 10> =
  UnionToIntersection<DotFold<T, Prev, "values", D>>
      ...
      ? (string extends K
          ...
          : (
              FormPathKey<K> extends never
                ? true
                : (T[K] extends FormDefinition ? _HasInvalidKeys<T[K]> : false)
            )
        )
      : false
}[keyof T]
...
export type AssertValidFormKeysDeep<T extends FormDefinition> =
  true extends _HasInvalidKeys<T>
    ? never
