...
const accountForm = defineForm({
        ...
        password: {
                ...
                defaultValue: "",
                validate: z.string().min(8, "Passwords must have at least 8 characters."),
        },
})
                ...
                <form {...handlers} className="space-y-6">
                        <Fieldset className="space-y-6">
                                ...
                                <Field name={username.name} invalid={Boolean(username.error)}>
                                        <FieldLabel htmlFor={username.name}>{username.label}</FieldLabel>
                                        <FieldControl>
                                                <input
                                                        id={username.name}
                                                        name={username.name}
                                        ...
                                        <FieldError id={username.errorId}>{username.error}</FieldError>
                                </Field>
                                ...
                                <Field name={email.name} invalid={Boolean(email.error)}>
                                        ...
                                        <FieldControl>
                                                <input
                                                        ...
                                                        aria-describedby={describedBy(email.describedById, email.errorId)}
                                        ...
                                        <FieldDescription id={email.describedById}>{email.description}</FieldDescription>
                                        <FieldError id={email.errorId}>{email.error}</FieldError>
                                </Field>

                                <Field name={password.name} invalid={Boolean(password.error)}>
                                        <FieldLabel htmlFor={password.name}>{password.label}</FieldLabel>
                                        <FieldControl>
                                                <input
                                                        id={password.name}
                                                        name={password.name}
